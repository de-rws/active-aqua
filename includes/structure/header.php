...
add_action( 'genesis_doctype', 'rws_do_doctype' );
/**
...
add_action( 'wp_head', 'rws_fetch_dns', 1 );
/**
...
add_action( 'wp_enqueue_scripts', 'rws_load_assets' );
/**
...
function rws_load_assets() {
	...
	wp_enqueue_style( 'rws', $stylesheet_dir . $src, array(), $assets_version );

	// Google Fonts
 	wp_enqueue_style(
 		'google-fonts',
 		'//fonts.googleapis.com/css?family=Lato:400,900,700,400italic,700italic',
 		array(),
 		null
 	);
	...
	add_filter( 'script_loader_src', 'rws_jquery_local_fallback', 10, 2 );

	// Main script file (in footer)
	$src = $use_production_assets ? '/build/js/scripts.min.js' : '/build/js/scripts.js';
	wp_enqueue_script( 'rws', $stylesheet_dir . $src, array('jquery'), $assets_version, true );
	wp_localize_script(
		'rws',
		'grunticon_paths',
		array(
	...
	// wp_localize_script( 'rws', 'ajax_object', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );

}

add_filter( 'style_loader_tag', 'rws_ie_style_conditionals', 10, 2 );
/**
 ...
 * @since 1.x
 */
function rws_ie_style_conditionals( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'rws_ie_script_conditionals', 10, 3 );
/**
 * Conditionally load jQuery v1 on old IE.
 *
 * @since 2.3.1
 */
function rws_ie_script_conditionals( $tag, $handle, $src ) {
 ...
 * @since 2.0.20
 */
add_action( 'wp_head', 'rws_jquery_local_fallback' );
function rws_jquery_local_fallback( $src, $handle = null ) {
...
// add_filter( 'genesis_pre_load_favicon', 'rws_pre_load_favicon' );
/**
 * Simple favicon override to specify your favicon's location.
 *
 * @since 2.0.0
 */
function rws_pre_load_favicon() {
...
// add_action( 'wp_head', 'rws_load_favicons' );
/**
...
add_action( "genesis_header_right", "rws_header_cta");
function rws_header_cta() {
	?>
	<a href="tel:+1-360-904-3497" class="btn header-cta">CALL US!</a>
	<?php
}
