...
add_filter( 'gallery_style', 'rws_gallery_style' );
/**
 * Remove the injected styles for the [gallery] shortcode.
 *
 * @since 1.x
 */
function rws_gallery_style( $css ) {
...
add_filter( 'the_content_more_link', 'rws_more_tag_excerpt_link' );
/**
 ...
 * @since 2.0.16
 */
function rws_more_tag_excerpt_link() {

	return ' <a class="more-link" href="' . get_permalink() . '">' . __( 'Read more &rarr;', CHILD_THEME_TEXT_DOMAIN ) . '</a>';

}

add_filter( 'excerpt_more', 'rws_truncated_excerpt_link' );
add_filter( 'get_the_content_more_link', 'rws_truncated_excerpt_link' );
/**
 ...
 * @since 2.0.16
 */
function rws_truncated_excerpt_link() {
...
// add_filter( 'genesis_post_info', 'rws_post_info' );
/**
...
// add_filter( 'genesis_post_meta', 'rws_post_meta' );
/**
 ...
 * @since 2.0.0
 */
function rws_prev_link_text( $text ) {

	return html_entity_decode('&#10216;') . ' ';

}

add_filter( 'genesis_next_link_text', 'rws_next_link_text' );
/**
 ...
 * @since 2.0.0
 */
function rws_next_link_text( $text ) {
 ...
 * @since 2.2.18
 */
add_filter( 'the_password_form', 'rws_password_form' );
function rws_password_form( $post = 0 ) {
...
add_filter( 'the_content', 'rws_highlight_non_breaking_spaces' );
/*
 * Highlight non-breaking spaces in drafts to give the author a chance to correct them
 *
 * @since 2.3.8
 */
function rws_highlight_non_breaking_spaces( $content ) {
