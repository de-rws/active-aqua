...
// add_action( 'genesis_theme_settings_metaboxes', 'rws_remove_theme_settings_metaboxes' );
/**
 ...
 * @since 2.0.0
 */
function rws_remove_theme_settings_metaboxes( $_genesis_theme_settings_pagehook ) {
...
add_filter( 'genesis_theme_settings_defaults', 'rws_theme_settings_defaults' );
/**
 * Set default values for custom theme options.
 *
 * @since 2.3.0
 */
function rws_theme_settings_defaults( $defaults ) {

	$defaults['rws_production_on']         = false;
	$defaults['rws_assets_version']        = null;
...
add_action( 'genesis_settings_sanitizer_init', 'rws_settings_sanitizer' );
/**
 * Set filters for custom theme options.
 *
 * @since 2.3.0
 */
function rws_settings_sanitizer() {
...
add_action( 'genesis_theme_settings_metaboxes', 'rws_theme_settings_metaboxes' );
/**
 * Add meta boxes for custom theme options.
 *
 * @since 2.3.0
 */
function rws_theme_settings_metaboxes( $pagehook ) {

	add_meta_box(
		'rws-environment-settings',
		__( 'Environment', CHILD_THEME_TEXT_DOMAIN ),
		'rws_environment_settings_box',
		$pagehook,
		'main',
		'high'
	);
 ...
 * @since 2.3.0
 */
function rws_environment_settings_box() {

	?>
	<p>
		<label>
			<input type="checkbox" name="<?php echo GENESIS_SETTINGS_FIELD; ?>[rws_production_on]" value="1" <?php checked( genesis_get_option('rws_production_on'), 1 ); ?> >
	...
	<p>
		<label>
			...
			<input type="text" name="<?php echo GENESIS_SETTINGS_FIELD; ?>[rws_assets_version]" value="<?php echo esc_attr( genesis_get_option('rws_assets_version') ); ?>" class="regular-text" id="<?php echo GENESIS_SETTINGS_FIELD; ?>[rws_assets_version]">
		</label>
	</p>
