...
// add_action( 'admin_enqueue_scripts', 'rws_load_admin_assets' );
/**
 * Enqueue admin CSS and JS files.
 *
 * @since 2.3.2
 */
function rws_load_admin_assets() {
	...
	wp_enqueue_style( 'rws-admin', $stylesheet_dir . $src, array(), null );

	$src = $use_production_assets ? '/build/js/admin.min.js' : '/build/js/admin.js';
	wp_enqueue_script( 'rws-admin', $stylesheet_dir . $src, array('jquery'), null, true );

}

add_action( 'pre_ping', 'rws_disable_self_pings' );
/**
 ...
 * @since 2.0.0
 */
function rws_disable_self_pings( &$links ) {
...
/**
 * Change the default image link to none instead of attachment page
 */
...
// add_filter( 'image_size_names_choose', 'rws_image_size_names_choose' );
/**
 ...
 * @since 2.0.0
 */
function rws_image_size_names_choose( $sizes ) {
 ...
 * @since 2.2.24
 */
function rws_get_image_sizes( $size = '' ) {
 ...
 * @since 2.3.6
 */
// add_filter( 'wp_generate_attachment_metadata', 'rws_downsize_uploaded_image', 99 );
function rws_downsize_uploaded_image( $image_data ) {
 ...
 * @since 2.2.3
 */
add_filter( 'xmlrpc_methods', 'rws_remove_xmlrpc_pingback_ping' );
function rws_remove_xmlrpc_pingback_ping( $methods ) {
 ...
 * @since 2.2.26
 */
add_action( '_core_updated_successfully', 'rws_remove_files_on_upgrade' );
function rws_remove_files_on_upgrade() {
